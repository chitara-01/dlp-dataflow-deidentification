...
#gradle run -DmainClass=com.google.swarm.tokenization.DLPTextToBigQueryStreamingV2 -Pargs="--region=us-central1 --project=${{env.PROJECT_ID}} --streaming --enableStreamingEngine --tempLocation=gs://${{env.GCS_BUCKET}}/temp --numWorkers=2 --maxNumWorkers=3 --runner=DataflowRunner --filePattern=gs://${{env.GCS_BUCKET}}/*.csv --dataset=${{env.DATASET_ID}} --inspectTemplateName=${{env.INSPECT_TEMPLATE_PATH}} --deidentifyTemplateName=${{env.DEID_TEMPLATE_PATH}} --batchSize=200000 --DLPMethod=DEID"
